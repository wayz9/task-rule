<div class="hidden xl:block absolute -right-2 -top-2 h-40 w-40 overflow-hidden">
    <span class="absolute top-0 h-2 w-2 bg-primary-600"></span>
    <span class="absolute bottom-0 right-0 h-2 w-2 bg-primary-600"></span>
    <div
        class="absolute bottom-0 right-0 block w-[calc(100%*1.41421356237)] origin-bottom-right rotate-45 bg-primary-300 p-2 text-center text-sm leading-tight">
        <span class="text-xs font-semibold uppercase text-primary-700">
            App Version 0.1.0
        </span>
        <br>
        <span class="font-bold">Currently In Beta</span>
    </div>
</div>
