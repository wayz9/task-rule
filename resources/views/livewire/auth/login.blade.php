<div class="relative flex flex-1 flex-col overflow-hidden px-4 py-8 sm:px-6 lg:px-8">
    <div class="relative mx-auto flex w-full max-w-[25.75rem] flex-1 flex-col items-center justify-center pb-16 pt-12">
        ...
        <form x-data wire:submit.prevent="login"
            ...
            <input type="email" placeholder="Email address" wire:model="email" required autocomplete="email"
                class="block w-full rounded-lg border border-gray-200 bg-white px-3.5 py-2 text-gray-800 placeholder:text-gray-600 sm:text-sm/6 @error('email') email @enderror" />
            @error('email')
            ...
            <div class="mt-4 flex flex-col items-end">
                <input type="password" placeholder="Password" wire:model="password" required
                    class="block w-full rounded-lg border border-gray-200 bg-white px-3.5 py-2 text-gray-800 placeholder:text-gray-600 sm:text-sm/6 @error('password') input-error @enderror" />
                <a wire:navigate.hover href="{{ route('password.request') }}"
                    class="relative text-gray-700 hover:text-gray-600 mt-2 text-sm/6 font-semibold group">
                    Forgot password?
                    <span
